...
import { getPassport } from "@shared/server/repositories/engagement";
...
export default defineHandler(async (event) => {
  if (!(await rateLimit(event, "engagement-passport"))) return errorResponse("Rate limit exceeded", 429);
  ...
  try {
    ...
    return jsonResponse({ passport, wallet });
  } catch (err) {
