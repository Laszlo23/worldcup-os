...
import { listWalletTransactions } from "@shared/server/repositories/engagement";
...
export default defineHandler(async (event) => {
  if (!(await rateLimit(event, "engagement-wallet-tx", 40, 60_000))) {
  ...
  try {
    ...
    const transactions = await listWalletTransactions(user.id, 50);
