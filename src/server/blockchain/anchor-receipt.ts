...
export async function verifyMemoTx(params: {
  ...
  const signerIndex = accountKeys.findIndex((k) => k.toBase58() === params.userPubkey);
  if (signerIndex < 0) return false;

  const logs = tx.meta?.logMessages ?? [];
  const memoLog = logs.find((l) => l.includes("Program log: Memo") || l.includes(params.expectedPrefix));
