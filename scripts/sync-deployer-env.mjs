...
const remoteScript = `const fs=require('fs');const patch=JSON.parse(fs.readFileSync('/tmp/wmos-env-patch.json','utf8'));const p='/var/www/wmos-buildingculture/.env';let e=fs.readFileSync(p,'utf8');for(const[k,v]of Object.entries(patch)){if(!v)continue;const re=new RegExp('^'+k+'=.*$','m');e=re.test(e)?e.replace(re,k+'='+v):e+'\\n'+k+'='+v;}fs.writeFileSync(p,e);console.log('ok',Object.keys(patch).join(','));`;
